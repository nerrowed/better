...
export default function ListFinaleExamRooms() {
  const navigate = useNavigate();
  const [finaleExamRooms, setFinaleExamRooms] = useState<FinaleExamRoomsRecord[]>([]);
  const { getRecords, isLoading, deleteRecord } = usePocketbase('finale_exam_rooms');
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between my-2">
        ...
        <div className="flex space-x-2">
          <Button variant="default" onClick={() => navigate('/finale-examination/rooms/c')}>
            <Plus /> Tambah Manual
          </Button>
        </div>
      </div>
      <SimpleDataTable
        ...
        onDelete={async data => {
          try {
            if (!confirm(`Are you sure you want to delete this "${data.room_name}"?`)) return;
